...
const startMarker = '"Kamu adalah AI learning designer untuk mahasiswa farmasi.",';
const endMarker = '].join("\\n\\n");';

const startIndex = content.indexOf(startMarker);
if (startIndex !== -1) {
  // Find the exact return [ that precedes this
  const returnIndex = content.lastIndexOf('return [', startIndex);
  const endIndex = content.indexOf(endMarker, startIndex) + endMarker.length;
  
  if (returnIndex !== -1 && endIndex !== -1) {
    const oldBlock = content.substring(returnIndex, endIndex);
